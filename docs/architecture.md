...
The smaller tension gauge uses D3 only for pure scale/arc calculations; React owns its entire SVG. It receives the same 5 Hz sample snapshot and configuration as the live readings. It does not interpolate between readings or create its own acquisition loop. Static scale/zone geometry depends on configured limits; the needle depends on absolute tension, independently of the differential baseline.
